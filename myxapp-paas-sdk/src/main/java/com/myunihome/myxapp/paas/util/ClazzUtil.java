...
public class ClazzUtil {
    ...
    public static Class<?> getInterfaceClazz(String interfaceName) throws Exception {
        ...
        try {
            ...
            throw new PaasRuntimeException("接口名[" + interfaceName + "]对应的类不存在，请检查");
        }
        if (interfaceClass == null) {
            throw new PaasRuntimeException("根据接口名[" + interfaceName + "]获取到的接口实例对象为空，请检查");
        }
        if (!interfaceClass.isInterface()) {
            throw new PaasRuntimeException("类[" + interfaceName + "]不是一个接口类，请检查定义");
        }
    ...
    public static List<java.lang.reflect.Method> getMethods(Class<?> interfaceClass,
        ...
        // 接口不能为空
        if (interfaceClass == null) {
            throw new PaasRuntimeException("接口类不能为空");
        }
        // 检查接口类型必需为接口
        if (!interfaceClass.isInterface()) {
            throw new PaasRuntimeException("类[" + interfaceClass.getName() + "]不是一个接口类，请检查定义");
        }
        // 检查方法是否在接口中存在
