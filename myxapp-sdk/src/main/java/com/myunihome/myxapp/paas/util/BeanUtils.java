...
public final class BeanUtils {
    ...
    public static void copyVO(Object destSVO, Object orignSVO) {
        ...
        if (destSVO == null || orignSVO == null) {
            throw new PaasRuntimeException("拷贝VO属性值出错:源对象为空或目标对象为空");
        }
        /* 2.获取目标对象所有字段 */
        ...
        for (Field field : fields) {
            ...
            if (!"serialVersionUID".equals(fieldName)) {
                try {
                    ...
                    throw new PaasRuntimeException("拷贝VO属性值出错:SecurityException", e);
                } catch (NoSuchFieldException e) {
                    throw new PaasRuntimeException("拷贝VO属性值出错:NoSuchFieldException", e);
                } catch (IllegalArgumentException e) {
                    throw new PaasRuntimeException("拷贝VO属性值出错:IllegalArgumentException", e);
                }
    ...
    public static Object getVoFieldValue(Object object, String fieldName) {
        try {
            if (object == null || StringUtil.isBlank(fieldName)) {
                throw new PaasRuntimeException("底层获取对象指定属性值出错,、对象为空或者指定字段为空");
            }
            ...
            throw new PaasRuntimeException("系统错误[BeanCopyUtil.getVoFieldValue]:" + retMsg, ex);
        }
    }
    ...
    public static void setVoFieldValue(Object object, String fieldName, Object fieldValue) {
        try {
            if (object == null || StringUtil.isBlank(fieldName)) {
                throw new PaasRuntimeException("设置对象指定属性值出错,对象为空或者指定字段为空");
            }
            ...
            String retMsg = ex.getCause() == null ? ex.getMessage() : ex.getCause().getMessage();
            throw new PaasRuntimeException("系统错误[BeanCopyUtil.setVoFieldValue]:" + retMsg, ex);
        }
    }
    ...
    public static Object getFieldValue(Object orignVo, String fieldName) {
        if (orignVo == null || StringUtil.isBlank(fieldName)) {
            throw new PaasRuntimeException("底层获取vo指定字段值出错,vo对象为空或者指定字段为空");
        }
        try {
            ...
            throw new PaasRuntimeException("获取属性值报错", ex);
        }
    }
